...
namespace ValeoBot.Services.ValeoApi
{
    public class ValeoAPIService : IValeoAPIService
    {
        ...
        {
            string json = await _client.GetStringAsync(string.Format(_config.ValeoApi.TimeUrl, doctor))
                .ConfigureAwait(false);
        ...
        {
            var response = await _client.PostAsJsonAsync(_config.ValeoApi.SaveUrl, order)
                .ConfigureAwait(false);
